...
The wind information for the main level 
```100.0, 70.0, 50.0, 30.0, 20.0, and 10.0 hpa``` 
(if available) are directly decoded from the sounding.
...
Currently these are:
  ...
  ```singapore2023.dat``` -  high resolution QBO for the current year (2023)
  ```singapore2022.dat``` -  high resolution QBO for the last year (2022)
    ```singapore2021.dat``` -  high resolution QBO for the last incomplete year to catch up (2021)
...
and ```singapore2020.dat``` have to be replaced by ```singapore2023.dat``` and ```singapore2022.dat```.
...
parse.sh
 -> parse_wind (parse_wind.f90)
    Decode the ```sing<yyyy><mm><dd>.<hh>``` files.
